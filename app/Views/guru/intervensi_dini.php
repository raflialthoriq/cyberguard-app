...
<body class="pb-36 font-sans text-gray-700 container mx-auto px-4 lg:max-w-5xl min-h-screen pt-8 relative">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div class="neu-flat p-5 rounded-3xl border-l-4 <?= $siswa['is_processed'] ? 'border-green-500' : ($siswa['keparahan'] == 'high' ? 'border-red-500' : 'border-yellow-500') ?>">
                        ...
                        <?php if ($siswa['is_processed']): ?>
                            <div class="mt-2 p-3 bg-green-50 border border-green-200 rounded-xl text-center shadow-inner">
                                <span class="text-xs font-black text-green-600 flex items-center justify-center gap-2">
        ...
        <div class="neu-flat p-5 md:p-8 rounded-3xl h-fit">
            ...
            <div class="mb-6 border-b pb-2 flex justify-between items-center">
                <h3 class="font-extrabold text-sm text-gray-800 uppercase tracking-wider flex items-center gap-2"><i data-lucide="calendar-days" class="w-4 h-4 text-blue-600"></i>
                    Agenda
                </h3>
                <div class="flex items-center gap-3 bg-white px-3 py-1.5 rounded-xl shadow-sm border border-gray-200">
                    <a href="?bln=<?= sprintf('%02d', $prev_bln) ?>&thn=<?= $prev_thn ?>" class="text-gray-400 hover:text-blue-600 font-bold transition"><i data-lucide="chevron-left" class="w-4 h-4"></i></a>
                    <span class="text-blue-600 font-black text-xs w-24 text-center"><?= $nama_bulan_aktif ?></span>
                    <a href="?bln=<?= sprintf('%02d', $next_bln) ?>&thn=<?= $next_thn ?>" class="text-gray-400 hover:text-blue-600 font-bold transition"><i data-lucide="chevron-right" class="w-4 h-4"></i></a>
                </div>
            </div>
        ...
        <div class="bg-[#E0E5EC] w-full max-w-md rounded-3xl shadow-2xl border border-white overflow-hidden transform scale-100 transition-transform">
            ...
            <div class="bg-white px-6 py-4 border-b flex justify-between items-center">
                ...
                <button onclick="tutupModal()"
                    class="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-gray-600 hover:bg-red-100 hover:text-red-600 transition">
    ...
    <nav class="fixed bottom-0 left-0 w-full bg-[#E0E5EC] rounded-t-3xl border-t border-white/50 z-50" style="box-shadow: 0 -10px 25px rgba(163,177,198,0.4);">
        <div class="max-w-5xl mx-auto px-2 md:px-6 py-3 flex justify-between items-center text-center">
            ...
            <a href="/guru/intervensi_dini" class="flex flex-col items-center text-red-600 hover:text-red-500 transition transform hover:-translate-y-1 w-1/6"><i data-lucide="shield-alert" class="w-5 h-5 md:w-6 md:h-6 mb-1"></i>
    ...
    <script>
        function bukaDetailJadwal(dataJadwal, tanggal) {
            ...
            const namaBulan = new Date().toLocaleString('id-ID', {
                month: 'long',
                year: 'numeric'
            });
                ...
                let cardHTML = `
                    <div class="${bgColor} border-l-4 p-4 rounded-xl shadow-sm relative overflow-hidden">
                        <div class="flex justify-between items-center mb-3">
                            <h4 class="font-black text-gray-800 text-base flex items-center gap-2"><i data-lucide="clock-3" class="w-4 h-4 text-blue-600"></i>${jam}:${menit} WIB</h4>
                        ...
                        <div class="space-y-1">
                            ...
                            <p class="text-xs font-bold text-indigo-600 mb-2 flex items-center gap-2">    <i data-lucide="map-pin" class="w-3 h-3"></i>${j.catatan}</p>
