    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-extrabold text-gray-800 flex items-center gap-2">
                <i data-lucide="gamepad-2" class="w-7 h-7 text-orange-500"></i>
                Laporan Simulasi
            </h1>
            <p class="text-teal-600 font-bold flex items-center gap-2">
                <i data-lucide="layers-3" class="w-4 h-4"></i>
                Level: <?= esc($skenario['judul_skenario'] ?? 'CBT') ?>
            </p>
        </div>
        <a href="/admin/kelola_simulasi"
            class="neu-flat px-4 py-2 rounded-xl text-xs font-bold text-gray-500 flex items-center gap-2 hover:text-blue-600">
        ...
        <table class="w-full text-left text-sm">
            <thead>
                <tr class="text-xs uppercase tracking-widest text-gray-500 border-b-2 border-gray-300">
                    ...
                    <th class="py-3 px-4">Waktu Simulasi</th>
            ...
            <tbody>
                <?php foreach ($partisipan as $p): ?>
                    <tr class="border-b border-gray-300/50 hover:bg-white/30 transition">
                        <td class="py-3 px-4 font-bold text-gray-800"><?= esc($p['nama_lengkap']) ?></td>
                        <td class="py-3 px-4 text-xs font-bold text-gray-600"><?= esc($p['nama_sekolah'] ?? '-') ?></td>
                        <td class="py-3 px-4 text-xs"><?= date('d M Y, H:i', strtotime($p['tanggal_percobaan'])) ?></td>
                        <td class="py-3 px-4 font-black <?= $p['skor_kontrol_diri'] > 0 ? 'text-green-500' : 'text-red-500' ?>">
                            <span class="px-3 py-1 rounded-lg flex items-center gap-1 w-fit <?= $p['skor_kontrol_diri'] > 0 ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600' ?>">

                                <?php if ($p['skor_kontrol_diri'] > 0): ?>
                                    <i data-lucide="trending-up" class="w-3 h-3"></i>
                                <?php else: ?>
                                    <i data-lucide="trending-down" class="w-3 h-3"></i>
                                <?php endif; ?>

                                <?= $p['skor_kontrol_diri'] > 0 ? '+' : '' ?>
                                <?= $p['skor_kontrol_diri'] ?> Poin
                            </span>
                        </td>
                    </tr>
                <?php endforeach; ?>
                        ...
                        <td colspan="4" class="py-8">
                            <div class="flex flex-col items-center justify-center text-gray-400 gap-2">
                                ...
                                <span class="font-bold">
                                    Belum ada siswa yang memainkan simulasi ini.
                                </span>
                            </div>
                        </td>
                    </tr>
