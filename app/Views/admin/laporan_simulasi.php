...
<body class="pb-28 p-6 font-sans text-gray-700 container mx-auto lg:max-w-4xl">
    <div class="flex justify-between items-center mb-6">
        <div><h1 class="text-2xl font-extrabold text-gray-800">Laporan Simulasi 🎮</h1><p class="text-teal-600 font-bold">Level: <?= esc($skenario['judul_skenario'] ?? 'CBT') ?></p></div>
        <a href="/admin/kelola_simulasi" class="neu-flat px-4 py-2 rounded-xl text-xs font-bold text-gray-500">⬅️ Kembali</a>
    </div>
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-300/50 hover:bg-white/30 transition">
                    ...
                    <td class="py-3 px-4 font-black <?= $p['skor_kontrol_diri'] > 0 ? 'text-green-500' : 'text-red-500' ?>">
                        <span class="px-3 py-1 rounded-lg <?= $p['skor_kontrol_diri'] > 0 ? 'bg-green-100' : 'bg-red-100' ?>">
                            <?= $p['skor_kontrol_diri'] > 0 ? '+' : '' ?><?= $p['skor_kontrol_diri'] ?> Poin
                        </span>
                    </td>
                </tr>
                <?php endforeach; ?>
                ...
                <?php if(empty($partisipan)): ?>
                    <tr><td colspan="4" class="py-8 text-center font-bold text-gray-400">Belum ada siswa yang memainkan simulasi ini.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
