...
<body class="pb-28 p-6 font-sans text-gray-700 container mx-auto lg:max-w-3xl">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-xl font-extrabold text-gray-800 flex items-center gap-2"><i data-lucide="clipboard-check" class="w-6 h-6 text-blue-600"></i>Detail Evaluasi Kuis</h1>
            <p class="text-sm font-bold">Siswa: <span class="text-blue-600"><?= esc($info['nama_lengkap']) ?></span> (Skor: <?= $info['skor_kuis'] ?>%)</p>
        </div>
        <a href="/admin/laporan_modul/<?= $info['id_modul'] ?>" class="neu-flat px-4 py-2 rounded-xl text-xs font-bold text-gray-500 flex items-center gap-2 hover:text-blue-600"><i data-lucide="arrow-left" class="w-4 h-4"></i>Kembali</a>
    </div>

    <div class="space-y-4">
        <?php $no = 1;
        foreach ($detail_jawaban as $d): ?>
            <div class="neu-flat p-5 rounded-2xl border-l-4 <?= $d['is_benar'] ? 'border-green-500' : 'border-red-500' ?>">
                <h3 class="font-bold text-sm text-gray-800 mb-2"><?= $no ?>. <?= esc($d['pertanyaan']) ?></h3>
                <div class="neu-pressed p-3 rounded-xl text-xs space-y-1">
                    <p class="font-bold <?= $d['is_benar'] ? 'text-green-600' : 'text-red-600' ?>">
                        Jawaban Siswa: <?= esc($d['jawaban_siswa']) ?><?php if ($d['is_benar']): ?>
                        <i data-lucide="check-circle" class="inline w-4 h-4 ml-1"></i>
                    <?php else: ?>
                        <i data-lucide="x-circle" class="inline w-4 h-4 ml-1"></i>
                    <?php endif; ?>
                    </p>
                    <?php if (!$d['is_benar']): ?>
                        <p class="font-bold text-blue-600 flex items-center gap-2"><i           data-lucide="key-round" class="w-4 h-4"></i>Kunci Benar: <?= esc($d['jawaban_benar']) ?></p>
                    <?php endif; ?>
                </div>
            </div>
        <?php $no++;
