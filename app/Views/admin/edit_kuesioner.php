...
<body class="pb-28 font-sans text-gray-700 container mx-auto px-4 lg:max-w-3xl min-h-screen pt-8">
    ...
    <div class="flex justify-between items-center mb-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-2xl font-extrabold text-gray-800">Edit Kuesioner</h1>
            </div>
        </div>
        <a href="/admin/kelola_kuesioner"
            class="neu-flat px-4 py-2 rounded-xl text-xs font-bold text-gray-500 hover:text-gray-800 transition active:scale-95 flex items-center gap-2">
        ...
        <form action="/admin/update_kuesioner/<?= $kuesioner['id_kuesioner'] ?>" method="POST">
                ...
                <div>
                    ...
                    <div class="relative">
                        <select name="status_aktif" class="w-full neu-pressed px-4 py-3 rounded-xl focus:outline-none text-sm font-bold text-gray-700 appearance-none cursor-pointer">
                            <option value="1" <?= $kuesioner['status_aktif'] == 1 ? 'selected' : '' ?>> Aktif</option>
                            <option value="0" <?= $kuesioner['status_aktif'] == 0 ? 'selected' : '' ?>> Nonaktif</option>
                        </select>
                        <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-4 text-gray-500 font-bold"><i data-lucide="chevron-down" class="w-4 h-4"></i></div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-orange-100 border-l-4 border-orange-500 p-4 rounded-xl mb-8">
                <p class="text-xs font-bold text-orange-700">
                    <span class="text-lg"><i data-lucide="triangle-alert" class="w-5 h-5 text-orange-600 flex-shrink-0 mt-0.5"></i></span>
            ...
            <button type="submit"
                class="w-full bg-blue-500 text-white font-extrabold py-4 rounded-xl shadow-lg hover:bg-blue-600 transition active:scale-95 text-sm md:text-base flex items-center justify-center gap-2"><i data-lucide="save" class="w-5 h-5"></i>
                Simpan Perubahan
            </button>
        </form>
    </div>
