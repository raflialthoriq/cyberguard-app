...
<body class="pb-28 p-6 font-sans text-gray-700 container mx-auto lg:max-w-4xl">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-extrabold text-gray-800 flex items-center gap-2"><i data-lucide="book-open-text" class="w-7 h-7 text-blue-600"></i>Laporan Modul</h1>
            <p class="text-blue-600 font-bold"><?= esc($modul['judul_modul']) ?></p>
        </div>
        <a href="/admin/kelola_modul" class="neu-flat px-4 py-2 rounded-xl text-xs font-bold text-gray-500 flex items-center gap-2 hover:text-blue-600"><i data-lucide="arrow-left" class="w-4 h-4"></i>Kembali</a>
    </div>
        ...
        <table class="w-full text-left text-sm">
            <thead>
                <tr class="text-xs uppercase text-gray-500 border-b-2 border-gray-300">
                    ...
                    <th class="py-3 px-2">Skor Kuis</th>
            ...
            <tbody>
                <?php foreach ($partisipan as $p): ?>
                    <tr class="border-b border-gray-300/50">
                        <td class="py-3 px-2 font-bold"><?= esc($p['nama_lengkap']) ?><br>
                            <span class="text-[10px] text-gray-500"><?= esc($p['nama_sekolah']) ?>
                            </span>
                        </td>
                        <td class="py-3 px-2">
                            <span class="px-2 py-1 text-[10px] rounded-md <?= $p['status_modul'] == 'selesai' ? 'bg-green-100 text-green-600' : 'bg-orange-100 text-orange-600' ?>"><?= strtoupper($p['status_modul']) ?></span>
                        </td>
                        <td class="py-3 px-2 font-black <?= $p['skor_kuis'] >= 70 ? 'text-green-500' : 'text-red-500' ?>"><?= $p['skor_kuis'] ?>%
                        </td>
                        <td class="py-3 px-2">
                            <a href="/admin/detail_jawaban_kuis/<?= $p['id_progres'] ?>"class="bg-blue-500 text-white px-3 py-2 rounded-lg text-[10px] font-bold inline-flex items-center gap-1"><i data-lucide="eye" class="w-3 h-3"></i>
                                Lihat
                            </a>
                        </td>
                    </tr>
