...
<body class="pb-32 font-sans text-gray-700 container mx-auto px-4 lg:max-w-4xl min-h-screen pt-8">
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-sm font-bold text-gray-500">
                    Kalimat afirmasi positif yang tampil di beranda siswa.
                </p>
            </div>
        </div>
    </div>
    ...
    <?php if (session()->getFlashdata('pesan')): ?>
        <div class="bg-green-100 text-green-700 p-4 rounded-xl mb-6 text-sm font-bold neu-flat flex items-center gap-2"><i data-lucide="check-circle" class="w-5 h-5"></i>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-1 neu-flat p-6 rounded-3xl h-fit">
            <h2 class="font-extrabold text-gray-800 mb-4 border-b-2 border-gray-300 pb-2 flex items-center gap-2"><i data-lucide="plus-circle" class="w-5 h-5 text-blue-500"></i>
                Tambah Afirmasi Baru</h2>
            <form action="/admin/simpan_tips" method="POST">
                ...
                <button type="submit"
                    class="w-full bg-blue-500 hover:bg-blue-600 text-white font-extrabold py-3 rounded-xl shadow-lg transition active:scale-95 flex items-center justify-center gap-2">
        ...
        <div class="lg:col-span-2 neu-flat p-6 rounded-3xl">
            <h2 class="font-extrabold text-gray-800 mb-4 border-b-2 border-gray-300 pb-2 flex items-center gap-2"> <i data-lucide="list" class="w-5 h-5 text-blue-500"></i>
                Daftar Afirmasi </h2>
                ...
                <?php foreach ($daftar_tips as $t): ?>
                    <div class="neu-pressed p-4 rounded-2xl flex justify-between items-center gap-4 group">
                        <div class="flex items-start gap-3">
                            <i data-lucide="quote" class="w-5 h-5 text-blue-400 mt-1 flex-shrink-0"></i>
                            <p class="text-sm font-bold text-gray-700 italic">
                                <?= esc($t['isi_tips']) ?>
                            </p>
                        </div>
                        <a href="/admin/hapus_tips/<?= $t['id_tips'] ?>"
                            onclick="return confirm('Hapus tips ini?')"
                            class="text-red-400 hover:text-red-600 opacity-50 group-hover:opacity-100 transition" title="Hapus"> <i data-lucide="trash-2" class="w-5 h-5"></i>
                        </a>
                    </div>
                <?php endforeach; ?>
        ...
        <div class="max-w-5xl mx-auto px-2 md:px-6 py-3 flex justify-between items-center text-center">
            ...
            <div class="flex-1 relative flex flex-col items-center cursor-pointer group" onclick="document.getElementById('menuAkses').classList.toggle('hidden'); event.stopPropagation();">
                ...
                <div id="menuAkses" class="hidden absolute bottom-full left-1/2 transform -translate-x-1/2 mb-3 bg-white rounded-2xl shadow-xl border border-gray-200 w-36 py-2 flex flex-col z-50 transition-all">
                    ...
                    <a href="/admin/kelola_tips" class="flex items-center px-4 py-3 text-xs font-bold text-blue-600 hover:text-blue-600 hover:bg-blue-50 transition border-b border-blue-100">
